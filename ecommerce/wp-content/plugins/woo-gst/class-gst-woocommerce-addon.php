...
class WC_GST_Settings {
    ...
    public static function init() {
    	add_filter( 'woocommerce_settings_tabs_array', __CLASS__ . '::fn_add_settings_tab', 50 );
    	add_action( 'woocommerce_settings_tabs_settings_gst_tab', __CLASS__ . '::fn_settings_tab' );
    	add_action( 'woocommerce_update_options_settings_gst_tab', __CLASS__ . '::fn_update_settings' );
    	add_action( 'woocommerce_update_options_tax', __CLASS__ . '::fn_update_tax_settings' );
    	add_action( 'init', __CLASS__ . '::fn_gst_callback' );
    	add_action('woocommerce_product_options_general_product_data', __CLASS__ . '::fn_add_product_custom_meta_box');
		add_action( 'woocommerce_process_product_meta',  __CLASS__ . '::fn_save_license_field');
        add_action( 'admin_print_scripts',  __CLASS__ . '::fn_load_custom_wp_admin_script', 999 );
	    add_action( 'woocommerce_email_after_order_table', __CLASS__ . '::fn_woocommerce_gstin_invoice_fields' );
    }
    ...
    public static function gst_insrt_tax_slab_rows() {
        ...
        if( get_option( 'woo_gst_data_set' ) == 'true'){
            return;
        }

    	$a_multiple_slabs = array();
    	if( isset( $_POST['woocommerce_product_types'] ) && $_POST['woocommerce_product_types'] == 'multiple' ){
            $multi_select_slab = sanitize_text_field( $_POST['woocommerce_gst_multi_select_slab'] );
    	...
    	foreach ( $a_multiple_slabs as $a_multiple_slab ) {
    		if( $a_multiple_slab != '0%' ){
                ...
                if( isset( $state ) ) :
    	    		$wpdb->insert($table_prefix,array( 'tax_rate_country' => 'IN', 'tax_rate_state' => $state,'tax_rate' => $state_tax,'tax_rate_name' => $state_tax."% CGST",'tax_rate_priority' => 1,'tax_rate_compound' => 1,'tax_rate_shipping' => 0,'tax_rate_order' => 0,'tax_rate_class' =>$slab_name),array( '%s','%s','%s','%s','%d','%d','%d','%d','%s'));

    	    		$wpdb->insert($table_prefix,array( 'tax_rate_country' => 'IN', 'tax_rate_state' => $state,'tax_rate' => $state_tax,'tax_rate_name' => $state_tax."% SGST",'tax_rate_priority' => 2,'tax_rate_compound' => 1,'tax_rate_shipping' => 0,'tax_rate_order' => 0,'tax_rate_class' =>$slab_name),array( '%s','%s','%s','%s','%d','%d','%d','%d','%s'));
        ...
        update_option( 'woo_gst_data_set', 'true' );
    }
