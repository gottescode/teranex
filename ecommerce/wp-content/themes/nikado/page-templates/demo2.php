...
if(count($jscomposer_templates) > 0) {
	foreach($jscomposer_templates as $jscomposer_template){
		if($jscomposer_template->post_title == 'Header Second' || $jscomposer_template->post_title == 'Footer First'){
		...
		<div class="page-wrapper">
			 	...
			 	<div class="header"> 
					<div class="header-content">
						...
						if ( isset($nikado_opt['header_layout']) && $nikado_opt['header_layout']!="") {
							...
							if(count($jscomposer_templates) > 0) {
								foreach($jscomposer_templates as $jscomposer_template){
									if($jscomposer_template->post_title == 'Header Second'){
			...
			<div class="footer footer-first">
				...
				if ( isset($nikado_opt['footer_layout']) && $nikado_opt['footer_layout']!="" ) {
					...
					if(count($jscomposer_templates) > 0) {
						foreach($jscomposer_templates as $jscomposer_template){
							if($jscomposer_template->post_title == 'Footer First'){
								echo do_shortcode($jscomposer_template->post_content);
							}
						}
					}
