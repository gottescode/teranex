...
class Customer extends FRONTEND_Controller {
    ...
    public function chat_with_us() {
        ...
        $chatlist = $this->customer_model->chatListHistory();
        $this->template->load("chat_with_us");
    }

    public function getListChat() {

        $url = site_url() . "customer/api/chatListHistory/";
    ...
    public function getChatUnique() {

        $chatID = $_POST['chatid'];
        //echo $chatID;die;
        $pageData['chatid'] = $chatID;

        //  print_r($pageData['chatid']);die;
        $url = site_url() . "customer/api/ChatUnique/";
     ...
     */
      public function eventAssignAttendee($eventId) {
        ...
        $userId = $this->session->userdata('uid');
         ...
         $url = site_url() . "/customer/api/fetchsingleventbooking/$eventId";
          $eventdata = apiCall($url, "get");
           $event_id=$eventdata['result'][0]['event_id'];
           
          $url = site_url() . "/customer/api/fetchsinglevent/$event_id";
          $singleeventdata = apiCall($url, "get");

        //print_r($singleeventdata);die;
        if ($_POST['btnSubmit']) {
           ...
           //print_r($pageData);die;
            ...
            $pageData['assign_by_user'] = $userId;
            $ce_id=$pageData['ce_id'];
            
            //echo $ce_id;die;
             $url = site_url() . "/customer/api/fetchsingleventbooking/$ce_id";
             $eventdata = apiCall($url, "get");
           

            
           
           $pageData['assign_date']=$eventdata['result'][0]['event_start_date'];
           $pageData['start_time']=$eventdata['result'][0]['event_start_time'];
           $pageData['end_time']=$eventdata['result'][0]['event_end_time'];
           $pageData['event_id']=$eventdata['result'][0]['event_id'];
        ...
        // get data event booking
        $url = site_url() . "/customer/api/eventbookingList/$userId";
        $eventbookingList = apiCall($url, "get");

           
        
        $assingUrl = site_url() . "customer/api/courseEnrollmentAssignList/$eventId";

        $courseEnrollmentAssignList = apiCall($assingUrl, "get");

        $arrayData = array(
            "attendeeList" => $attendeeList['result'],
            "courseData" => $eventbookingList['result'],
            "courseEnrollmentAssignList" => $courseEnrollmentAssignList['result'],
            "eventdata" => $eventdata['result'],
            "singleeventdata" => $singleeventdata['result'],
            "eventId" =>$eventId,
...
// Store this sessionId in the database for later use
       ...
       // echo $sessionId;die;
        ...
        //print_r($pageData);die;
         ...
         $url = site_url()."customer/api/start_event_organizer";
         $response=apiCall($url, "post",$pageData);
         //print_r($response);die;
    ...
    $returnData = array("sessionId"=>$sessionId,"tokenId"=>$token,"API_KEY"=>$API_KEY);
        ...
        public function opentok($evenID) { 
             $url = site_url() . "/customer/api/findSingleJoinEvent/$evenID";
           $result = apiCall($url, "get");
            ...
            // print_r($arrayData);die;
           $this->load->view('opentok',$arrayData);
        ...
        public function close_event()
                {
                       $pageData = $this->input->post();
                   //   print_r($pageData);die;
                      $url = site_url()."customer/api/close_event";
                      $response=apiCall($url,"post",$pageData);
                    //$this->customer_model->close_event($pageData);
