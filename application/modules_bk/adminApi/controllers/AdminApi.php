...
class AdminApi extends REST_Controller
{
    ...
    public function blockUser_post(){
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $this->email->send();
            $this->email->from('noreplay@im-messenger.com', 'MyBabyAdmin');
            $this->email->to($this->security->xss_clean($to_email));
            $this->email->subject('Account deactivation');
            $this->email->message($message);
            $this->email->send();
            $this->Admin_Model->blockUser($this->post('userId',true));
            $response = array(
                "status" => array(
                    "code" => REST_Controller::HTTP_OK,
                    "message" => "ok"
    ...
    public function updateAdmin_post(){
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            if (!$this->User_Model->ifExist($this->post('userEmail'))) {
                ...
                $this->Admin_Model->updateAdmin($this->post('userId',true),$this->post('userName',true),$this->post('userEmail',true),$this->post('userPass',true),$role);
                $response = array(
                    "status" => array(
                        "code" => REST_Controller::HTTP_OK,
                        "message" => "ok"
