...
//	}	
	...
	public function chatMsgInsert_post() {
		// get input data
		$data = $this->post();
		$result = $this->machine_model->chatMsgInsert($data);
		if($result){
			$response = [ "result" => $result, "message" => "Chat save successfully.!!!!" ];
		} else {
			$response = [
				"result" => false,
				'message' => "Faild to add request."
	...
	public function chatMsgHistory_post() {
		// get input data
		$data = $this->post();
		$result = $this->machine_model->chatMsgHistory($data);
		$this->response($result, REST_Controller::HTTP_OK);
    }
