...
class Allusers_model extends CI_Model {
	...
	public function findMultiple($strWhere) {
		$result=$this->db_lib->fetchMultiple("master_user as MU LEFT JOIN user_details UD ON MU.uid = UD.uid", $strWhere." ORDER BY MU.uid DESC" ,"MU.*,UD.*,MU.uid as id");//exit; 
		return $result;
	}

	public function usertype($strWhere) {
		$result=$this->db_lib->fetchMultiple("user_type_master", $strWhere,'');//exit; 
		return $result;
	}
	public function userrole($strWhere) {
		$result=$this->db_lib->fetchMultiple("role_master", $strWhere,'');//exit; 
		return $result;
	}
