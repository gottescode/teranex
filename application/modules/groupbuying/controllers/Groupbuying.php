...
class Groupbuying extends FRONTEND_Controller {
	...
	*/ 
	...
	/* Sheet Metal Data */
		...
		if(isset($_POST['servicepart_offerSubmit'])){
				$pageData = $this->input->post();
				
				$pageData['owner_id'] = $this->session->userdata('uid');
				$url = site_url()."groupbuying/adminapi/createServicePartOffer/";
				$response =  apiCall($url, "post",$pageData);
				
				if($response['result']){
					setFlash("dataMsgSuccess", " Your Offer has been submitted Successfully..!! ");
					redirect(site_url()."groupbuying");
				}else{
					setFlash("dataMsgError", 'Something Went Wrong..!!');
					redirect(site_url()."groupbuying");
		...
		if(isset($_POST['consumableofferSubmit'])){
				$pageData = $this->input->post();	
				$pageData['owner_id'] = $this->session->userdata('uid');
				$url = site_url()."groupbuying/adminapi/createConsumableOffer/";
				$response =  apiCall($url, "post",$pageData);
		...
		if(isset($_POST['sheetMetalOfferSubmit'])){
				$pageData = $this->input->post(); 
				$pageData['owner_id'] = $this->session->userdata('uid');	
				$url = site_url()."groupbuying/adminapi/createSheetMetalOffer/";
				$response =  apiCall($url, "post",$pageData);
