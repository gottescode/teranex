...
class Consumable_model extends CI_Model {
	...
	public function rfqDetails($strWhere){
		...
		$result=$this->db_lib->fetchMultiple("consumable_customer_rfq as ccr LEFT JOIN mst_consumables_name as mcn ON mcn.id=ccr.cons_name_id LEFT JOIN mst_consumables_category as mcc  ON ccr.cons_category_id = mcc.id LEFT JOIN mst_consumables_type as mct ON mct.id = ccr.cons_type_id LEFT JOIN mst_consumables_brand as mcb ON mcb.id = ccr.cons_brand_id LEFT JOIN master_user as mu ON mu.uid = ccr.customer_id ", $strWhere,"ccr.id,cons_freq,cons_quantity,rfq_status,quote_price_by_admin,created_date,mcn.name as Consumablename,mcc.consumable_category,mct.consumable_type,mcb.consumable_brand,mu.u_name"); 
		return $result;
	}
	/* assign supplier  
		15/5/2018
