...
class Sheetmetal_model extends CI_Model {
    ...
    function __construct() {
	 ...
	 public function singleRfqDetailssheetmetal($strWhere = 1) {
		return $this->db_lib->fetchSingle('sheet_metal_admin_rfq', $strWhere,'');
	}
	 
	public function sheetmetalCustrequestList($strWhere) {
		$result=$this->db_lib->fetchMultiple(" sheet_metal_customer_rfq as spcr LEFT JOIN mst_sheet_metal_part_name as mspn ON mspn.id=spcr.sheet_name_id LEFT JOIN mst_sheet_metal_part_brand as mspb ON mspb.id=spcr.sheet_brand_id LEFT JOIN mst_sheet_metal_part_category as mspc ON mspc.id=spcr.sheet_category_id LEFT JOIN 	mst_sheet_metal_part_type as mst ON mst.id= spcr.sheet_type_id LEFT JOIN mst_sheet_metal_part_thickness as msmpt ON msmpt.id=spcr.sheetmetal_thickness LEFT JOIN mst_sheet_metal_part_size as msmps ON msmps.id=spcr.sheetmetal_size LEFT JOIN master_user as mu ON  mu.uid=spcr.customer_id", $strWhere,"spcr.*,mspn.sheetmetal_name,mspb.sheetmetal_brand,mspc.sheetmetal_category,mst.sheetmetal_type,msmpt.sheetmetal_thickness,msmps.sheetmetal_size,mu.u_name");//exit; 
		return $result;	
	}
	public function supplierQuotationListsheetmetal($strWhere) {
		$result=$this->db_lib->fetchMultiple("sheet_metal_assigned_suppliers as cas LEFT JOIN master_user as mu ON  mu.uid=cas.supplier_id", $strWhere," cas.*,mu.u_name as supplier_name");//exit; 
		return $result;
	}
	...
	public function adminRfqListsheetmetal($arrayData){
		$result=$this->db_lib->fetchMultiple("sheet_metal_admin_rfq","",""); 
		return $result;
	}
	 ...
	 * @return array  
	 */
	public function assignSuppliersheetmetal($data){
			
		$adminRfqid=$this->db_lib->insert("sheet_metal_admin_rfq", $data); 
		// get total records
		if($adminRfqid){
			$ids = $data["id"]; 
		if(count($ids)>0){
			foreach($ids as $id){
				if($data["publish_$id"] == "Y"){
					...
					$old_result = $this->db_lib->fetchSingle('sheet_metal_assigned_suppliers', $strWhere,'');
					if($old_result==0){
						$result = $this->db_lib->insert("sheet_metal_assigned_suppliers", $arrData);
					}else{
						$arrayData['id'] = $old_result['id'];
						$arrayData['request_supplier_date'] =  date('Y-m-d');
						$result = $this->db_lib->update("sheet_metal_assigned_suppliers", $arrayData," id = ".$arrayData['id']);
					}
				}	
			}
			$updatedata['rfq_status'] = 'Y';
			$rfq_ids = $data['rfq_ids'];
			$result = $this->db_lib->update("sheet_metal_customer_rfq", $updatedata," id IN({$rfq_ids}) ");
	...
	public function updateQuotationSupplierPricesheetmetal($arrData) {
		
		$adminAmount = $arrData['admin_price'];
		$rfq_ids =$arrData['rfq_ids'];
		$sheet_quantity = $arrData['sheet_quantity'];
		$singleAmount = $adminAmount/$sheet_quantity;
		
		//get all customer results
		$strWhere = " id IN({$rfq_ids})  ";
		$result = $this->db_lib->fetchMultiple("sheet_metal_customer_rfq",$strWhere,""); 
	
		
		foreach($result as $row){
			//update the amount
			$amount = 0;
			$amount = round($singleAmount*$row['sheet_quantity']);
			$arrData1['quote_price_by_admin'] = $amount;
			$arrData1['quote_status'] = 'Y';
			$result = $this->db_lib->update("sheet_metal_customer_rfq", $arrData1, "id = " . $row["id"]);	
		}
		
			$adminData['admin_final_price'] = $adminAmount;
			$adminData['supplier_id'] = $arrData['supplier_id'];
			$admin_result = $this->db_lib->update("sheet_metal_admin_rfq", $adminData, "id = " . $arrData["admin_rfq_id"]);
			$adminUpData['admin_status'] = 'A';
			$adminUpData['admin_accepted_date'] = date('Y-m-d H:i:s');
			$adminUpData['admin_price'] = $adminAmount;
			$supplier_result = $this->db_lib->update("sheet_metal_assigned_suppliers", $adminUpData, "id = " . $arrData["id"]);
	...
	public function findMultipleSheetMetalCategory($strWhere) {
		$result=$this->db_lib->fetchMultiple("mst_sheetmetal_part_category", $strWhere,"");//exit; 
		return $result;
	}
	
	public function findMultipleSheetMetalBrand($strWhere) {
		$result=$this->db_lib->fetchMultiple("mst_sheetmetal_part_brand", $strWhere,"");//exit; 
		return $result;
	}
	public function findMultipleSheetMetalType($strWhere) {
		$result=$this->db_lib->fetchMultiple("mst_sheetmetal_part_type", $strWhere,"");//exit; 
		return $result;
	}
	public function findMultipleSheetMetalQty($strWhere) {
		$result=$this->db_lib->fetchMultiple("mst_sheetmetal_part_qty", $strWhere,"");//exit; 
		return $result;
	}
	public function findMultipleSheetMetalUnit($strWhere) {
		$result=$this->db_lib->fetchMultiple("mst_sheetmetal_part_unit", $strWhere,"");//exit; 
		return $result;
	}
