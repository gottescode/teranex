...
class Serviceparts_model extends CI_Model {
    ...
    function __construct() {
	 ...
	 public function singleRfqDetailsServiceParts($strWhere = 1) {
		return $this->db_lib->fetchSingle('service_parts_admin_rfq', $strWhere,'');
	}
	 
	public function servicepartsCustrequestList($strWhere) {
	...
	public function supplierQuotationListServiceParts($strWhere) {
		$result=$this->db_lib->fetchMultiple("service_parts_assigned_suppliers as cas LEFT JOIN master_user as mu ON  mu.uid=cas.supplier_id", $strWhere," cas.*,mu.u_name as supplier_name");//exit; 
		return $result;
	}
	...
	public function adminRfqListServiceParts($arrayData){
		$result=$this->db_lib->fetchMultiple("service_parts_admin_rfq","",""); 
		return $result;
	}
	...
	public function rfqDetailsServiceParts($strWhere){
		  
		$result=$this->db_lib->fetchMultiple(" service_parts_customer_rfq as spcr LEFT JOIN mst_service_part_name as mspn ON mspn.id=spcr.service_name_id LEFT JOIN mst_service_part_brand as mspb ON mspb.id=spcr.service_brand_id LEFT JOIN mst_service_part_category as mspc ON mspc.id=spcr.service_category_id LEFT JOIN mst_service_part_type as mst ON mst.id= spcr.service_type_id LEFT JOIN master_user as mu ON  mu.uid=spcr.customer_id", $strWhere,"spcr.*,mspn.servicepart_name,mspc.servicepart_category,mst.servicepart_type,mu.u_name,mspb.servicepart_brand"); 
		return $result;
	}
	/* assign supplier  
		15/5/2018
	 ...
	 * @return array  
	 */
	public function assignSupplierServiceParts($data){
			
		$adminRfqid=$this->db_lib->insert("service_parts_admin_rfq", $data); 
		...
		if(count($ids)>0){
			foreach($ids as $id){
				if($data["publish_$id"] == "Y"){
					...
					$old_result = $this->db_lib->fetchSingle('service_parts_assigned_suppliers', $strWhere,'');
					if($old_result==0){
						$result = $this->db_lib->insert("service_parts_assigned_suppliers", $arrData);
					}else{
						$arrayData['id'] = $old_result['id'];
						$arrayData['request_supplier_date'] =  date('Y-m-d');
						$result = $this->db_lib->update("service_parts_assigned_suppliers", $arrayData," id = ".$arrayData['id']);
					}
				}	
			}
			$updatedata['rfq_status'] = 'Y';
			$rfq_ids = $data['rfq_ids'];
			$result = $this->db_lib->update("service_parts_customer_rfq", $updatedata," id IN({$rfq_ids}) ");
	...
	public function updateQuotationSupplierPriceServiceParts($arrData) {
		
		$adminAmount = $arrData['admin_price'];
		$rfq_ids =$arrData['rfq_ids'];
		$service_quantity = $arrData['service_quantity'];
		$singleAmount = $adminAmount/$service_quantity;
		
		//get all customer results
		$strWhere = " id IN({$rfq_ids})  ";
		$result = $this->db_lib->fetchMultiple(" service_parts_customer_rfq",$strWhere,""); 
	
		
		foreach($result as $row){
			//update the amount
			$amount = 0;
			$amount = round($singleAmount*$row['service_quantity']);
			$arrData1['quote_price_by_admin'] = $amount;
			$arrData1['quote_status'] = 'Y';
			$result = $this->db_lib->update("service_parts_customer_rfq", $arrData1, "id = " . $row["id"]);	
		}
		
			$adminData['admin_final_price'] = $adminAmount;
			$adminData['supplier_id'] = $arrData['supplier_id'];
			$admin_result = $this->db_lib->update("service_parts_admin_rfq", $adminData, "id = " . $arrData["admin_rfq_id"]);
			$adminUpData['admin_status'] = 'A';
			$adminUpData['admin_accepted_date'] = date('Y-m-d H:i:s');
			$adminUpData['admin_price'] = $adminAmount;
			$supplier_result = $this->db_lib->update("service_parts_assigned_suppliers", $adminUpData, "id = " . $arrData["id"]);
