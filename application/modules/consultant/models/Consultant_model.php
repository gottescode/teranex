...
/*  Service List */
   ...
   public function findMultipleAddress($strWhere) {
	...
	/* Technical Services */
	public function findMultipleTechnicalServiceRequestList($strWhere) {
		$table = "  technical_service_request as tsr 
					...
					machine_brand_model as mbm ON md.model_no = mbm.md_id
						LEFT JOIN 
					master_user as Muu ON  tsr.service_engg_id = Muu.uid 
	
				";
		$select = " tsr.*,mu.u_name,md.md_id,md.machine_unique_id,md.category_id,model_no,mb.brand_name,mc.category_name,mbm.model_name,Muu.u_name as engg_name  ";
		
		$result = $this->db_lib->fetchMultiple($table, $strWhere." ORDER BY tsr.id DESC",$select); 
		return $result;
	}
	
	public function meetingUpdateStatus($data){
		return $result = $this->db_lib->update("technical_support_zoom", $data," id = ".$data['id']);
	}	
	public function meetingUpdateStatusApplication($data){
		return $result = $this->db_lib->update("application_support_zoom", $data," id = ".$data['id']);
	}	
	public function techServiceUpdateStatus($data){
		
		return $result = $this->db_lib->update("technical_service_request", $data," id = ".$data['id']);
	}	
	public function applicationUpdateStatus($data){
		
		return $result = $this->db_lib->update("application_support_request", $data," id = ".$data['id']);
	}	
	public function assignconsultantTrainingRequest($data){
		
		$data['status'] = 'W';
		$result = $this->db_lib->update("technical_service_request", $data," id = ".$data['tech_req_id']);
		if($result){
			$uid = $data['service_engg_id'];
			$emailData = $this->db_lib->fetchSingle('master_user',' uid = '.$uid,'');
			$to = $emailData['u_email'];
			$body = '<p>Hello '.$emailData['u_name'] . ',</p> ';
			$body .= 'Please check the Dashboard..!! You have new Technical Service Request..!';
			$email_content = $body;
			$this->load->library('Email_PHPMailer'); 
			$subject = 'Technical Service Request';
		...
		if(count($ids)>0){
			foreach($ids as $id){
				if($data["publish_$id"] == "Y"){
					$arrData["tech_req_id"] = $data['tech_req_id'];
					$arrData["service_engg_id"] = $id;
					$arrData["assigned_date"] = date('Y-m-d H:i:s');
					$tech_req_id = $arrData["tech_req_id"];
					$strWhere = " tech_req_id = $tech_req_id AND service_engg_id = '$id'";
					$old_result = $this->db_lib->fetchSingle('technical_assigned_service_engg', $strWhere,'');
					if($old_result==0){
						$result = $this->db_lib->insert("technical_assigned_service_engg", $arrData);
					}else{
						$arrayData['id'] = $old_result['id'];
						$arrayData['assigned_date'] =  date('Y-m-d H:i:s');
						$result = $this->db_lib->update("technical_assigned_service_engg", $arrayData," id = ".$arrayData['id']);
					}
				}	
			}
			return true;
...
/* Application Support */
	/* public function findMultipleApplicationRequestList($strWhere) {
		$table = " application_support_request as tsr LEFT JOIN master_user as mu ON tsr.customer_id = mu.uid ";
		$select = " tsr.*,mu.u_name ";
	...
	public function findMultipleApplicationRequestList($strWhere) {
		$table = "  application_support_request as tsr 
					...
					machine_brand_model as mbm ON md.model_no = mbm.md_id
						LEFT JOIN 
					master_user as Muu ON  tsr.engg_id = Muu.uid 
	...
	public function assignconsultantApplicationRequest($data){
		$data['status'] = 'W';
		$data['engg_id'] = $data['service_engg_id'];
		return $result = $this->db_lib->update("application_support_request", $data," id = ".$data['app_req_id']);
		/* 
		...
		if(count($ids)>0){
			foreach($ids as $id){
				if($data["publish_$id"] == "Y"){
					$arrData["app_req_id"] = $data['app_req_id'];
					$arrData["engg_id"] = $id;
					$arrData["assigned_date"] = date('Y-m-d H:i:s');
					$app_req_id = $arrData["app_req_id"];
					$strWhere = " app_req_id = $app_req_id AND engg_id = '$id'";
					$old_result = $this->db_lib->fetchSingle('application_support_assigned_service_engg', $strWhere,'');
					if($old_result==0){
						$result = $this->db_lib->insert("application_support_assigned_service_engg", $arrData);
					}else{
						$arrayData['id'] = $old_result['id'];
						$arrayData['assigned_date'] =  date('Y-m-d H:i:s');
						$result = $this->db_lib->update("application_support_assigned_service_engg", $arrayData," id = ".$arrayData['id']);
					}
				}	
			}
			return true;
...
/* Spare Request */
	public function findMultipleSpareRequestList($strWhere) {
		...
		";
		$select = " tsr.*,md.md_id,md.machine_unique_id,md.category_id,model_no,mu.u_name,mb.brand_name,mc.category_name,mbm.model_name ";
		
		$result = $this->db_lib->fetchMultiple($table, $strWhere." ORDER BY tsr.id DESC",$select); 
		return $result;
	}
	
	public function findSingleUserGetInfo($id){
		return $result  = $this->db_lib->fetchSingle("master_user"," uid = $id"," u_name");
	}
	public function findMultipleDeliveryList($id){
		return $result  = $this->db_lib->fetchMultiple("spare_part_delivery_tracker"," spare_req_id = $id","");
	}
	public function updateSpareOrderDetails($data){
		$id  = $data['id'];
		if($data['status']){
		...
		$result  = $this->db_lib->update("spare_part_request",$data1," id = {$id} ");
		if($result){
			$data2['spare_req_id'] = $data['id'];
			$data2['title'] = $data['title'];
			$data2['description'] = $data['description'];
			$result  = $this->db_lib->insert(" spare_part_delivery_tracker ",$data2);
		}
			return $result;
	}
	public function closeSpareOrderDetails($id){
		$data['status'] = 'C';
		
		return	$result  = $this->db_lib->update("spare_part_request",$data," id = {$id} ");
	}
	public function findMultipleSpareDetailsList($id){
		return	$result  = $this->db_lib->fetchMultiple("spare_request_images"," spare_req_id = {$id} ");
	}
	public function findMultipleFreelancerList($id){
		$strWhere = " 1 "; 
		  $table = " freelancer_support_request as tsr 
					...
					machine_brand_model as mbm ON md.model_no = mbm.md_id	 
						LEFT JOIN 
					master_user as muu ON tsr.freelancer_id = muu.uid 
					";
 
        $select = " tsr.*,mu.u_name as username,mu.u_mobileno as userMobile,mu.u_email as useremail,md.parent_id,machine_unique_id,md.category_id,model_no,mc.category_name,mbm.model_name,mb.brand_name,muu.u_name as ufree ";

       return $result = $this->db_lib->fetchMultiple($table, $strWhere, $select);
	}
}
