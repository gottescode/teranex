...
class Training extends FRONTEND_Controller {
	...
	public function trainerDetails($id) {
		...
		if($recently_viewed_data['result']){
			...
			if (!(in_array($id, $checkInarray))) {
					...
					$newData['user_id'] = $uid;
					$url = site_url()."consultant/api/updateRecentlyViewedTraining/";
			...
			$url = site_url()."consultant/api/createRecentlyViewedTraining/";
			$result = apiCall($url,"POST",$newData);
