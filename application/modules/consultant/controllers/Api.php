...
/* Create RemoteApplication Request */
	...
	/* Technical Services */
	public function findMultipleTechnicalServiceRequestList_get($id=0) { 
		$strWhere = $this->get("strWhere");
		if(!$strWhere) $strWhere = 1;
			
			$response = [
				"result" => $this->consultant_model->findMultipleTechnicalServiceRequestList($strWhere)
			];
		 	
		$this->response($response, REST_Controller::HTTP_OK);
	}
	
	public function assignconsultantTrainingRequest_post() {
			 
        /* $this->form_validation->set_rules('id[]', 'select Consultant', 'trim|required');  */
        $this->form_validation->set_rules('service_engg_id', 'Engineer', 'trim|required'); 
		if ($this->form_validation->run() == FALSE) {
            $response = [
                "result" => false,
                'message' => validation_errors()
			...
			$id = $this->consultant_model->assignconsultantTrainingRequest($data);
			if($id){
				$response = [ "result" => $id, "message" => "Consultant Assigned Successfully...!!" ];
			} else {
				$response = [
	...
	public function findMultipleApplicationRequestList_get($id=0) { 
		$strWhere = $this->get("strWhere");
		if(!$strWhere) $strWhere = 1;
			
			$response = [
				"result" => $this->consultant_model->findMultipleApplicationRequestList($strWhere)
			];
		 	
		$this->response($response, REST_Controller::HTTP_OK);
	}
	
	public function assignconsultantApplicationRequest_post() {
			 
        $this->form_validation->set_rules('service_engg_id', 'Service Engineer is not selected', 'trim|required'); 
        /* $this->form_validation->set_rules('id[]', 'select Consultant', 'trim|required');  */
		if ($this->form_validation->run() == FALSE) {
            $response = [
                "result" => false,
			...
			$id = $this->consultant_model->assignconsultantApplicationRequest($data);
			if($id){
				$response = [ "result" => $id, "message" => "Consultant Assigned Successfully...!!" ];
			} else {
				$response = [
	...
	public function findMultipleEnggList_get($id=0) { 
		$strWhere = 1;
		$strWhere .= " AND uid <>$id";
		$strWhere .= " AND (( user_type=1 AND (user_role=1 OR user_role=6 OR user_role=7)) OR ( user_type=3 AND (user_role=1 OR user_role=6  OR user_role=7)) OR ( user_type=4 AND (user_role=1  OR user_role=6 OR user_role=7)) )";
		$response = [
			"result" => $this->consultant_model->findMultipleConsultantUsersList($strWhere)
		];
	...
	/* Spare Request List */
	public function findMultipleSpareRequestList_get($id=0) { 
		$strWhere = $this->get("strWhere");
		if(!$strWhere) $strWhere = 1;
			
			$response = [
				"result" => $this->consultant_model->findMultipleSpareRequestList($strWhere)
			];
		 	
		$this->response($response, REST_Controller::HTTP_OK);
	}
	public function findMultipleSpareRequestListSupplier_get($id=0) { 
		$strWhere = $this->get("strWhere");
		if(!$strWhere) $strWhere = 1;
			$strWhere = " tsr.parent_id = $id ";
			$response = [
				"result" => $this->consultant_model->findMultipleSpareRequestList($strWhere)
			];
	...
	public function findSingleUserGetInfo_get($id=0) { 
		$strWhere = $this->get("strWhere");
		if(!$strWhere) $strWhere = 1;
			
			$response = [
				"result" => $this->consultant_model->findSingleUserGetInfo($id)
			];
		 	
		$this->response($response, REST_Controller::HTTP_OK);
	}

	public function updateSpareOrderDetails_post() {
			 
        $this->form_validation->set_rules('id', 'Enter Id', 'trim|required'); 
        $this->form_validation->set_rules('title', 'Enter Title', 'trim|required'); 
        $this->form_validation->set_rules('description', 'Enter Description', 'trim|required'); 
		if ($this->form_validation->run() == FALSE) {
            $response = [
                "result" => false,
                'message' => validation_errors()
			...
			$id = $this->consultant_model->updateSpareOrderDetails($data);
			if($id){
				$response = [ "result" => $id, "message" => "Update Successfully...!!" ];
			} else {
				$response = [
					"result" => false,
					'message' => "Failed to insert record."
	...
	public function findMultipleDeliveryList_get($id=0) {
		if($id){
			
			$response = [
				"result" => $this->consultant_model->findMultipleDeliveryList($id)
			];
	...
	public function closeSpareOrderDetails_get($id=0) {
		if($id){
			
			$response = [
				"result" => $this->consultant_model->closeSpareOrderDetails($id)
			];
	...
	public function findMultipleSpareDetailsList_get($id=0) {
		if($id){
			
			$response = [
				"result" => $this->consultant_model->findMultipleSpareDetailsList($id)
			];
	...
	/* Freelancer */
		public function findMultipleFreelancerList_get($id=0) {
	
			
			$response = [
				"result" => $this->consultant_model->findMultipleFreelancerList($id)
			];
		
			
		$this->response($response, REST_Controller::HTTP_OK);
	}
	public function meetingUpdateStatus_post() {
	 
        $this->form_validation->set_rules('meeting_status', 'meeting_status ', 'trim|required');
        $this->form_validation->set_rules('meeting_description', 'meeting_description ', 'trim|required');
		 ...
		 if ($this->form_validation->run() == FALSE) {
			...
			$page_id = $this->consultant_model->meetingUpdateStatus($data);
			if($page_id){
				$response = [ "result" => $page_id, "message" => "Record inserted successfully." ];
			} else {
				$response = [
	...
	public function meetingUpdateStatusApplication_post() {
        ...
        $this->form_validation->set_rules('meeting_description', 'meeting_description ', 'trim|required');
		 ...
		 if ($this->form_validation->run() == FALSE) {
			...
			$page_id = $this->consultant_model->meetingUpdateStatusApplication($data);
			if($page_id){
				$response = [ "result" => $page_id, "message" => "Record inserted successfully." ];
			} else {
				$response = [
	...
	public function techServiceUpdateStatus_post() {
	 
        $this->form_validation->set_rules('status', 'meeting_status ', 'trim|required');
		 ...
		 if ($this->form_validation->run() == FALSE) {
			...
			$page_id = $this->consultant_model->techServiceUpdateStatus($data);
			if($page_id){
				$response = [ "result" => $page_id, "message" => "Record inserted successfully." ];
			} else {
				$response = [
	...
	public function applicationUpdateStatus_post() {
        ...
        $this->form_validation->set_rules('status', 'meeting_status ', 'trim|required');
		 ...
		 if ($this->form_validation->run() == FALSE) {
			...
			$page_id = $this->consultant_model->applicationUpdateStatus($data);
			if($page_id){
				$response = [ "result" => $page_id, "message" => "Record inserted successfully." ];
			} else {
				$response = [
