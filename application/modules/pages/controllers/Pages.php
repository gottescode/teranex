        ...
        if (isset($_POST['otp_no'])) {
            ...
            if ($company_code_exist == 1) {
                ...
                if ($response['result'] == 1) {
                    ...
                    $body .= '<p>Click  <a href= "'.site_url().'" alt="Here">Here</a> to sign up to Stelmac.io platform with below details – </p>';
        ...
        if (isset($_POST['btnLogin'])) {
            ...
            if (!empty($pageData)) {
                ...
                if ($response['result']) {
                    ...
                    $session["u_parent_id"] = $response['result']['u_parent_id'];
                    // set extra varible
        ...
        if ($success) {
            ...
            try {
                ...
                if ($count > 0) {
                    ...
                    $this->pages_model->updateDetails('master_user', 'u_email', $user->email, $data);
                   ...
                   // $result = $this->db_lib->insert("ecommerce_users", $data_eco);
                    $id = $this->db_lib->insert("master_user", $data_master);
                    $first_user = 1;
                    $arrData = array('uid' => $id, 'GID' => $first_user);
                    $result = $this->pages_model->Add_data('user_details', $arrData);
                    if ($id > 0) {
                        $user_data["name"] = $user->given_name;
                        $user_data["email"] = $user->email;
