...
//    }
	...
	public function zoomResponseUpdateVideoChatRequest_post() {
        $this->form_validation->set_rules('start_url', 'Start Url is missing', 'trim|required');
        $this->form_validation->set_rules('join_url', 'Name Required', 'trim|required');
        $this->form_validation->set_rules('vcr_id', 'Name Required', 'trim|required');
        if ($this->form_validation->run() == FALSE) {
            $response = [
                "result" => false,
                'message' => validation_errors()
            ...
            // get input data
            $data = $this->post();
            $result = $this->pages_model->zoomResponseUpdateVideoChatRequest($data);
            if ($result) {
                $response = ["result" => $result, "message" => "Record updated successfully.!!!!"];
            } else {
                $response = [
    ...
    public function VideoChatRequestList_get() {


        $result = $this->pages_model->VideoChatRequestList();

        if ($result) {
            $response = ["result" => $result, "message" => "Video Chat Request Data get.!!!!"];
        } else {
            $response = [
                "result" => false,
                'message' => "Failed to retrive data."
