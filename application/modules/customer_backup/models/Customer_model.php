...
// get chat list
    ...
    public function chatListHistory() {
        $fromId = 0;
        $table = "chat_details as cd LEFT JOIN master_user as Mu ON Mu.uid = cd.msg_from";
        //$table = "chat_details ";
        $select = "cd.*,Mu.*";
        //$table = "chat_details ";
        //$select = "*";
        $strWhere = " cd.msg_to=$fromId AND cd.msg_from !=0";
        $result = $this->db_lib->fetchMultiple($table, $strWhere, $select);

//                       $table1 = "master_user ";
//		        $select = "*";
//                        $strWhere = "uid= {$result[0]['msg_from']}";
//                        $userResult = $this->db_lib->fetchSingle($table1,$strWhere,$select);
//                      
//                        $finalResult = array();
//                        $finalResult['result'] = $result;
//                         $finalResult['userData'] = $userResult;
        //   print_r($result);die;
...
//      public function ChatUnique($data) { 
//      //print_r($data);
//		$toId=$data["chatid"];
//		$fromId=0;
//		$table = "chat_details ";
//		$select = "*";
//		$strWhere = "(msg_from=0 AND msg_to=$toId) OR (msg_from=$toId AND msg_to=0)";
//		$result = $this->db_lib->fetchMultiple($table,$strWhere,$select);
//                return $result;
//    }
    ...
    public function ChatUnique($data) {
        ...
        //$table = "chat_details ";
        $select = "cd.*,Mu.*";
        $strWhere = "(cd.msg_from=0 AND cd.msg_to=$toId) OR (cd.msg_from=$toId AND cd.msg_to=0)";
        $result = $this->db_lib->fetchMultiple($table, $strWhere, $select);
        return $result;
    }
...
//print_r($arrUserData);exit;
  ...
  //   print_r($arrData);die;
    ...
    public function eventsList($id) {
        ...
        $result = $this->db_lib->fetchMultiple("event_booking EB JOIN event EE ON EB.event_id=EE.event_id", " EB.user_id = " . $id, "EE.event_name, EB.participant_no, EB.event_start_date, EB.event_start_time,eb_id");
    ...
    public function RapidprototypingRfqList($userId) {
        ...
        $strWhere = "customer_user_id = '$userId' AND RPRRS.admin_accept_status ='Y'";
        $result = $this->db_lib->fetchMultiple("rapid_prototyping_rfq RPR JOIN rapid_prototyping_rfq_request_supplier RPRRS ON RPRRS.dmr_id=RPR.dmr_id ", $strWhere, "RPR.*,RPRRS.*");
    ...
    public function LaserprocessingRfqList($userId) {
        ...
        $strWhere = "customer_user_id = '$userId' AND LPRRS.admin_accept_status ='Y'";
        $result = $this->db_lib->fetchMultiple("laser_processing_rfq LPR JOIN laser_processing_rfq_request_supplier LPRRS ON LPRRS.dmr_id=LPR.dmr_id ", $strWhere, "LPR.*,LPRRS.*");
...
/////////////////////////////    machine brand Model    //////////////////////////////////
    ...
    /***** Remote Application RFQ *****/
     ...
     public function findMultiplerfqlist($strWhere = 1) {
        $result = $this->db_lib->fetchMultiple('remote_application_rfq RAR LEFT JOIN remote_application_consultant_request_programmer RACRP ON  RACRP.rpr_id = RAR.rpr_id LEFT JOIN master_user MU ON  MU.uid = RACRP.applicationengineer_id', $strWhere . " ORDER BY rpr_id DESC", 'RACRP.request_status,MU.u_name,RAR.*');
