...
class analytics_model extends CI_Model {
	...
	public function reportCustomizationInsert($data) { 
		...
		$result = $this->db_lib->insert("analytics_report_customization",$data); 
        return $result;
    }
	...
	public function reportCustomizationList() { 
		$result = $this->db_lib->fetchMultiple("analytics_report_customization ARC LEFT JOIN analytics_case_study ACS ON ARC.case_study_id=ACS.case_study_id", "report_cust_id<>0 ORDER BY report_date","ARC.*, ACS.case_study_name"); 
        return $result;
    }
	 ...
	 */
     ...
     public function findSingleClient($strWhere = 1) {
		return $this->db_lib->fetchSingle('analytics_client', $strWhere,'');
	}
	 
	public function findMultipleClient($strWhere) {
		$result=$this->db_lib->fetchMultiple("analytics_client", $strWhere,"");//exit; 
		return $result;
	}
    ...
    public function createClient($arrData) {
		...
		return $result = $this->db_lib->insert("analytics_client", $arrData); 
    }
	...
	public function updateClient($arrData) {
		...
		$result = $this->db_lib->update("analytics_client", $arrData, "client_id = " . $arrData["id"]);
        return $result;
    }
	...
	public function updatePublishClient($data){
		...
		if(count($ids)>0){
			foreach($ids as $id){
				...
				//$arrData["display_order"]=$data["display_order_$id"];
				$result = $this->db_lib->update("analytics_client", $arrData, "client_id = ". $id);
			}
			return true;
		}
		return false;
