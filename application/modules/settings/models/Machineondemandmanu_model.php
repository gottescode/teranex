...
class Machineondemandmanu_model extends CI_Model {

    // constructor of this class
    function __construct() {
        // call parent constructor 
			$this->load->library("file_manager");
			$this->frontend_machine_on_rent="frontend_machine_on_demand_manu"; 
			// $this->frontend_machine_on_rent_software="frontend_machine_onrent_software"; 
			$this->frontend_machine_on_rent_software="frontend_machine_on_demand_manu_software"; 
			$this->frontend_machnine_on_demand_manu_main_category="	frontend_machnine_on_demand_manu_main_category"; 
			$this->frontend_machine_on_rent_sub ="frontend_machine_on_demand_manu_subdata"; 
			$this->frontend_path="uploads/machine_on_demand_manu_frontend_images"; 
			parent::__construct();
    }
	...
	public function createFront($arrData) {
		...
		$page_id = $this->db_lib->insert("frontend_machine_on_demand_manu",$arrData);
	...
	$page_id = $this->db_lib->insert("frontend_machine_on_demand_manu_software",$arrData);
	...
	public function findSingleMachineOnrentCat($strWhere = 1) {
		return $this->db_lib->fetchSingle($this->frontend_machnine_on_demand_manu_main_category, $strWhere,'');
	}
	public function findMultipleMachineOnrentCat($strWhere) {
		$result=$this->db_lib->fetchMultiple($this->frontend_machnine_on_demand_manu_main_category." ", $strWhere."","");//exit; 
		return $result;
	}
	public function createMachineOnrentCat($arrData) {
		$page_id = $this->db_lib->insert($this->frontend_machnine_on_demand_manu_main_category, $arrData);
		return $page_id ;
    }
	public function updateMachineOnrentCat($arrData) { 
		$result = $this->db_lib->update($this->frontend_machnine_on_demand_manu_main_category, $arrData, "id = " . $arrData["id"]);
        return $result;
    }
	public function deleteMachineOnrentCat($id) {
		$result = $this->db_lib->delete($this->frontend_machnine_on_demand_manu_main_category, "id = " . $id);
        return $result;
    }
	...
	$page_id = $this->db_lib->insert("frontend_machine_on_demand_manu_subdata",$arrData);
