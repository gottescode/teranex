...
class Exchangegroupapi extends API_Controller {
	...
	public function findSingleOfferedData_get($id) {	  
		    // get input data 
			$result = $this->exchangegroup_model->findSingleOfferedData($id);
			if($result){
				$response = [ "result" => $result, "message" => "Data.!!!!" ];
			} else {
				$response = [
	...
	public function offeredList_get($id) {	  
		    // get input data 
			$result = $this->exchangegroup_model->offeredList($id);
			if($result){
				$response = [ "result" => $result, "message" => "Data.!!!!" ];
			} else {
				$response = [
	...
	public function offeredListAdmin_get($id=1) {	  
		    // get input data 
			$result = $this->exchangegroup_model->offeredListAdmin($id);
			if($result){
				$response = [ "result" => $result, "message" => "Data.!!!!" ];
			} else {
				$response = [
	...
	public function offeredListFrontEnd_get($id=1) {	  
		    // get input data 
			$result = $this->exchangegroup_model->offeredListFrontEnd($id);
			if($result){
				$response = [ "result" => $result, "message" => "Data.!!!!" ];
			} else {
				$response = [
	...
	public function createOffer_post() {
		$this->form_validation->set_rules('category', 'Name Required', 'trim|required');
         if ($this->form_validation->run() == FALSE) {
		    ...
		    // get input data
			$data = $this->post();
			$result = $this->exchangegroup_model->createOffer($data);
			if($result){
				$response = [ "result" => $result, "message" => "Offer Added Successfully..!!" ];
			} else {
				$response = [
					"result" => false,
					'message' => "Something Went Wrong Please Try Again Later."
